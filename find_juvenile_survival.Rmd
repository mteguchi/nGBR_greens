...
define.matrix <- function(Ts, VT, f, phi.1, phi.J, phi.A){
  ...
  M[1, M.dim] <- f[2] * phi.A
  M[1, (M.dim - k[length(Ts)]):(M.dim - 1)] <- f[1] * phi.J[length(phi.J)]
...
Using all these information, we arrive at 0.445 (using the unlikely 0.95) or 0.30 (using the more likely but probably still too high 0.65). 

1 yr survival = 0.685 x 0.99 x 0.69 x 0.95 = 0.445

1 yr survival = 0.685 x 0.99 x 0.69 x 0.65 = 0.304
...
#### First year breeder
The fecundity for the first year breeder is computed as the following: Eggs/nests (103.9) x nests/female (6.2) x proportion of the 1st year breeder that nest (0.125) x sex ratio (see below). This is also an unknown as the maturing adult survival rate is unknown.  

Base: 103.9 x 6.2 x 0.125 x 0.8 = 64.4

Modelrate climate: 103.9 x 6.2 x 0.125 x 0.88 = 70.9

Extreme climate: 103.9 x 6.2 x 0.125 x 0.99 = 79.7
...
in.list <- list(Ts = c(5, 5, 19),   # average years in two juvenile stages and subadults
                VT =  c(1, 1, (1.9^2)),  # variance of years in two juvenile stages 
                f =  c(64.4, 96.3), # 
                phi.1 =  0.3,
                phi.A = 0.95,
                lambda = 0.95)
