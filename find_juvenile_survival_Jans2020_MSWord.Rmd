...
The fecundity for the maturing adult stage is computed as the following: Eggs/nest (103.9) x nests/female (6.2) x prop breed (0.382) x sex ratio (see below). This is also an unknown as the maturing adult survival rate is unknown.  

Proportion of breeding females is included.

Modelrate climate: 103.9 x 6.2 x 0.382 x 0.88 = 216.55

Extreme climate: 103.9 x 6.2 x 0.382 x 0.99 = 243.62
...
in.list <- list(Ts = Ts,   
                VT =  VT, 
                f =  c(216.55, 106.0), # includes prop breed but no survival included
                phi.1 =  0.4,
                phi.A = 0.95,
                lambda = 1.04)
...
if (save.file)
  write.table(M.moderate.104$M, 
              file = paste0("data/Moderate_104_", Sys.Date(), ".csv"),
              row.names = F,
              col.names = FALSE,
              sep = ",")
...
# find the juvenile survival and transition probabilities
estim.extreme.104 <- gosolnp(fun = obj.fcn.varstage,  # in nGBR_greens_fcns.R
...
if (save.file) 
  write.table(M.extreme.104$M, 
              file = paste0("data/Extreme_104_", Sys.Date(), ".csv"),
              row.names = F,
              col.names = FALSE,
              sep = ",")
